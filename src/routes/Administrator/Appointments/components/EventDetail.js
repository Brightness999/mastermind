...
export default class EventDetail extends React.Component {
	...
	render() {
		...
		const event = this.props.calendarEvents?.find(e => e._id === this.props.id);
			...
			<div className='calendar-content'>
				<div className='flex flex-row items-center'>
					...
					<div className='flex flex-row ml-auto'>
						<p className='font-18 font-700 mb-0 mr-5'>{intl.formatMessage(msgDashboard.recurring)}</p>
				...
				{!isConfirm && <div className='div-left-btn'>
					...
					<Button onClick={this.props.backView}>
						{intl.formatMessage(msgDashboard.cancelEvent).toUpperCase()}
					</Button>
				</div>}
