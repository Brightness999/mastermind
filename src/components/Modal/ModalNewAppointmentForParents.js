...
class ModalNewAppointmentForParents extends React.Component {
	...
	createAppointment = (data) => {
		const { dependents, fileList, selectedTimeIndex, selectedDate, selectedSkillSet, address, selectedDependent, selectedProvider, arrTime, notes, listProvider, selectedProviderIndex, standardRate } = this.state;
		...
		const subsidies = dependent?.subsidy?.filter(s => s.skillSet === selectedSkillSet && s.status === 5 && selectedProvider === s.selectedProviderFromAdmin)?.sort((a, b) => new Date(a.approvalDate) > new Date(b.approvalDate) ? -1 : 1);
	...
	onConfirmEvaluation = () => {
		...
		const { fileList, selectedTimeIndex, selectedDate, selectedSkillSet, address, selectedDependent, selectedProvider, arrTime, notes, listProvider, selectedProviderIndex, duration } = this.state;
		const { years, months, date } = selectedDate.toObject();
		const hour = arrTime[selectedTimeIndex]?.value.clone().set({ years, months, date });
		const postData = {
			skillSet: selectedSkillSet,
	...
	onChooseProvider = (providerIndex) => {
		const { dependents, listProvider, selectedDate, selectedDependent, selectedSkillSet } = this.state;
		...
		if (selectedDependent) {
			...
			if (selectedSkillSet) {
				const dependent = dependents?.find(d => d._id === selectedDependent);
				const subsidies = dependent?.subsidy?.filter(s => s.skillSet === selectedSkillSet && s.status === ADMINAPPROVED && s.selectedProviderFromAdmin === listProvider[providerIndex]._id)?.sort((a, b) => new Date(a.approvalDate) > new Date(b.approvalDate) ? -1 : 1);
				if (subsidies?.length) {
	...
	handleSelectDependent = (dependentId) => {
		const { dependents, searchKey, user } = this.state;
		this.setState({
					...
					{loading ? (
						...
						<Form onFinish={() => appointmentType === SCREEN ? this.onOpenModalScreening() : this.createAppointment()} layout='vertical' ref={ref => this.form = ref}>
							...
							<Row className='justify-end gap-2 mt-10'>
								...
								{user.role === PROVIDER && (appointmentType === APPOINTMENT || appointmentType === SUBSIDY) && listProvider[selectedProviderIndex]?.isSeparateEvaluationRate ? (
									<Dropdown.Button
