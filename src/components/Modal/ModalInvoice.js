	...
	componentDidMount() {
		...
		if (event?.sessionInvoice) {
			...
			const initItems = [{
				...
				count: event?.type === SUBSIDY ? `[${(appointments?.filter(a => a?.type === SUBSIDY && a.subsidy?._id === event?.subsidy?._id && [PENDING, CLOSED].includes(a?.status) && a?.dependent?._id === event?.dependent?._id && a?.provider?._id === event?.provider?._id)?.sort((a, b) => moment(a.date) > moment(b.date) ? 1 : -1)?.findIndex(a => a._id === event?._id) || 0) + 1}/${event?.subsidy?.numberOfSessions}]` : '',
